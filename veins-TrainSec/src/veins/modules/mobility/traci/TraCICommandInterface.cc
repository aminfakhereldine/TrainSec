...
void TraCICommandInterface::Vehicle::setDeccel(double deccel)
{
    uint8_t variableId = VAR_DECEL;
    uint8_t variableType = TYPE_DOUBLE;
    TraCIBuffer buf = traci->connection.query(CMD_SET_VEHICLE_VARIABLE, TraCIBuffer() << variableId << nodeId << variableType << deccel);
    ASSERT(buf.eof());
}
...
double TraCICommandInterface::Vehicle::getEmergencyDecel()
{
    return traci->genericGetDouble(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_EMERGENCY_DECEL, RESPONSE_GET_VEHICLE_VARIABLE);
}
